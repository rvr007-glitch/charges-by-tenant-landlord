  ...
  jwt.verify(req.token, config.SECRET_KEY, (err,authData) => {
    if(err)return sendError(res,err,constants.JWT_VERIFY)
        ...
        try {
            ...
            if(!occupation){
                return sendError(res,"Occupation of address can't be empty",500)
            }
            if(!account.acc_num){
    ...
    if(typeof bearerHeader !== 'undefined') {
      ...
      jwt.verify(req.token, config.SECRET_KEY, (err,authData) => {
        if(err)return sendError(res,err,constants.JWT_VERIFY)
            ...
            Landlord.findByIdAndUpdate(authData.id,{$set:req.body,},function(err,data){
                if(err)return sendError(res,err.message,constants.UPDATE_ERROR)
